  ...
  grunt.registerMultiTask('loopback_angular_addModelData_fork', 'Extends grunt-loopback-angular. Auto-generated Angular $resource services for loopback will include model data for use at runtime on angular clients', function () {
    ...
    for (var modelName in models) {
      if (models[modelName].public) {
        ...
        try {
          var modelFilePath;
          for (var i = 0; i < models._meta.sources.length; i++) {
            if (fs.existsSync(path.resolve(models._meta.sources[i] + '\\' + modelJSONFileName))) {
              modelFilePath = path.resolve(models._meta.sources[i] + '\\' + modelJSONFileName);
              grunt.log.ok('Find JSON file for ' + modelName + ' ' + modelFilePath);
            }
          }
